...
States with no effective voter ID requirement offer **7.7x more welfare benefits** on average compared to states requiring ID verification.
...
Three versions of the US map are provided, each using a different grouping of voter ID strictness:

- **5-tier** (`state_map.png`): Full NCSL classification
- **3-tier** (`state_map_3tier.png`): Strict / Non-Strict / Weak-None
- **2-tier** (`state_map_2tier.png`): ID Required vs No Effective ID
...
For the primary analysis, these are collapsed into two groups based on **functional outcome**:
